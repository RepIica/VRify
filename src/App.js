...
class App extends Component {
    ...
    if (localStorage.getItem("token")) {
      getCurrentUser(localStorage.getItem("token")).then(user => {
        if (user) {
          ...
          getProjects(user.id).then(projectsData => {
  ...
  promptProjectName = () => {
    let projName = prompt(
      `Please enter project name, ${this.props.currentUser.name}`
    );
    if (projName) {
      // validations
      ...
      <React.Fragment>
        ...
        <Switch>
          ...
          <Route
            path="/"
            render={() => {
              return this.props.currentUser ? <Ui /> : <Redirect to="/login" />;
            }}
          />
        </Switch>
      </React.Fragment>
